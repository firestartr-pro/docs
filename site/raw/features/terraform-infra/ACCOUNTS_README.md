...
# Example
./terrafire.sh -f account1 dev vpc 12345abcd-6789-ef01-2345-6789abcdef00 -- -force
```
...
# For example, the state of the VPC module in production ('pro') for account 'client-a'...
./terrafire.sh -p client-a pro vpc

# And to test the VPC and the EKS modules in dev environment for account 'client-b'...
./terrafire.sh -p client-b dev vpc eks

# And to test all modules in the predev environment for account 'client-c'...
...
# For example, creating the VPC in production (pro) for account 'client-a'...
./terrafire.sh -a client-a pro vpc

# And to create the VPC and the EKS in development (dev) for account 'client-b'...
./terrafire.sh -a client-b dev vpc eks

# And to apply all microstates in the 'testing' environment for account 'client-c'...
...
# For example, deleting the VPC in production (pro) for account 'client-a'...
./terrafire.sh -d client-a pro vpc

# And to delete the VPC and the EKS in 'dev' environment for account 'client-b'...
./terrafire.sh -d client-b dev vpc eks

# And to delete all microstates in 'test' environment for account 'client-c'...
...
# For example, unlocking the state 12345abcd-6789-ef01-2345-6789abcdef00 for the VPC in production for account 'client-a'
...
# For example, show details for the VPC in production for account 'client-a'
./terrafire.sh -s client-a pro vpc show -- 'aws_subnet.public'

# And for removing resources for the state in the EKS in dev environment for account 'client-b'...
...
# For example, importing an VPC in the production environment into the vpc module for account 'client-a'
./terrafire.sh -i client-a pro vpc aws_subnet.public subnet-1234567890abcdef0

# And to import the EKS in dev environment for account 'client-b'...
./terrafire.sh -i client-b dev eks aws_eks_cluster.main eks-0fedcba0987654321
```



# Terrafire script reference

This section outlines the usage of the `terrafire.sh` script for managing Terraform deployments.

## 1. Synopsis

The `terrafire.sh` script is designed to execute Terraform commands (`plan`, `apply`, `destroy`, `force-unlock`) across different accounts, environments, and modules.
...
* `<SCRIPT_DIR>`: The directory where `terrafire.sh` is located.

* `<account_name>`: A directory representing an AWS account.

* `<environment_name>`: A subdirectory within an account, representing an environment (e.g., `dev`, `pro`). This folder needs the files `variables.tf` and `providers.tf` in order for the system to function properly. The `providers.tf` file cannot be placed directly in its final location because its path depends on the account and environment names. In the next section we will explain how this file should be generated.
...
As previously discussed, we cannot provide the providers.tf as it is dependent on both the **account** name and the **environment** name, so we can have different providers for each environment. Each environment needs its own `providers.tf` file.
...
For example, if you have a "my_name" account and "dev" environment, the path of this file should be `accounts/my_name/dev/providers.tf`.

To prepare the file, you can change the **AWS region**, and (this is the important part) the **AWS account ID** for the **account** you are adding this file to.
...
provider "aws" {
   ...
   dynamic "assume_role_with_web_identity" {
    ...
    content {
      role_arn = "arn:aws:iam::<AWS_ACCOUNT_ID>:role/tf-base-role"  # add here the account's AWS account ID
  ...
  assume_role {
    role_arn = "arn:aws:iam::<AWS_ACCOUNT_ID>:role/AccountAdminRole" # add here the account's AWS account ID
  }
...
Also, we have provided a sample `examples/env/providers.tf` file that you can use to add to your current configuration in the right place, in your own account/environment folders. This file is only a template, and can be modified to suit your needs.
...
```bash
./terrafire.sh OPTION ACCOUNT ENVIRONMENT [MODULE] [-- EXTRA_ARGS]
Arguments:
...
ACCOUNT: (Required) The name of the account. This must correspond to a directory at the same level as the script.
Example: my-account

ENVIRONMENT: (Required) The name of the environment. This must correspond to a subdirectory within the specified ACCOUNT folder.

Example: dev, staging, pro
MODULE...: (Optional) The name(s) of specific module(s) to target. These must correspond to subdirectories within the specified ACCOUNT/ENVIRONMENT folder. If omitted, the action applies to all modules within the environment. You can specify multiple modules separated by spaces.
...
Displaying Available Accounts and Modules:
If an invalid account or no account is provided, the script will list available accounts.
If an invalid option or environment is provided (after a valid account), the script will attempt to list available modules for that account and environment.

### 5. Examples
Plan all modules in the dev environment for account-alpha:

Apply the vpc module in the pro environment for account-beta:

Destroy the eks and network modules in the staging environment for account-gamma:
...
Plan the webserver module in dev for account-delta and pass an extra variable to Terraform:

## 6. Terraform Backend and Variables

Backend Configuration: The script expects a backend.tf file in <SCRIPT_DIR> for backend configuration. It dynamically sets the key for the S3 backend based on account, environment, and module.
Provider Configuration: A providers.tf file can be placed in <SCRIPT_DIR>/<account>/<environment>/ to be copied into the working directory for the Terraform operations.
...
<SCRIPT_DIR>/<account>/account.tfvars
<SCRIPT_DIR>/<account>/<environment>/environment.tfvars
<SCRIPT_DIR>/<account>/<environment>/<module>/terraform.tfvars
...
`firestartr/<account>/<environment>/<module>`

If you use multiple GitHub repositories with the terraform-infra feature installed, this structure may cause collisions if the `<account>/<environment>/<module>` path is repeated across repositories.

To avoid this, you can define a custom prefix by creating a `.firestartr_tfstate_prefix` file. The value in this file will be inserted between `firestartr/` and `<account>`, resulting in the following structure: `firestartr/<custom_value>/<account>/<environment>/<module>`
