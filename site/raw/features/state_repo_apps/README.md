...
- `kubernetes` folder structure: `<platform>`/`<tenant>`/`<environment>`. At the same level as the `environment` folder, an `<environment_name>.yaml` file is expected, which contains the [Helmfile release configuration](KUBERNETES_README.md#helmfile-configuration) for rendering the Helm charts.
